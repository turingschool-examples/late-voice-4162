...
RSpec.describe 'Items Index Page', type: :feature do
  describe 'As a visitor' do
    ...
    it 'shows all items prices, number of customers that have purchased them, and the supermarket they belong to' do
      within "#item-#{@item_1.id}" do
        expect(page).to have_content(@item_1.price)
        expect(page).to have_content(@supermarket_1.name)
        expect(page).to have_content("Purchased by 1 customer(s)")
      end
      ...
      within "#item-#{@item_3.id}" do
        ...
        expect(page).to have_content(@supermarket_1.name)
        expect(page).to have_content("Purchased by 3 customer(s)")
      end
      ...
      within "#item-#{@item_5.id}" do
        ...
        expect(page).to have_content(@supermarket_2.name)
        expect(page).to have_content("Purchased by 0 customer(s)")
      end
    end
  end
end